...
import { fetchList, createAdmin, updateAdmin, deleteAdmin, ModifyAdminStatus } from "@/api/admin";
...
export default {
  ...
  methods: {
    ...
    handleModifyStatus(row, status) {
      ...
      ModifyAdminStatus(request_data).then((response)=>{
    ...
    createData() {
      ...
      this.$refs["dataForm"].validate((valid) => {
        if (valid) {
          this.temp.user_id = parseInt(Math.random() * 100) + 1024; // mock a id  随机产生一个ID  //这些操作应放在后端，只是测试使用
          this.temp.create_user_id = this.id
          createAdmin(this.temp).then((response) => {
            this.dialogFormVisible = false; // 关闭对话框
            if (response.code == 20000){
              this.temp.user_id = response.data
    ...
    handleDelete(row, index) {
      ...
      this.$confirm('此操作将永久删除该数据, 是否继续?', '提示', {
          ...
          var request_data = {
            id: this.temp.user_id
          }
          deleteAdmin(request_data).then(response => {
            this.$notify({
              title: '成功',
              message: '成功删除',
              type: 'success',
    ...
    handleDownload() {
      ...
      import("@/vendor/Export2Excel").then((excel) => {
        const tHeader = ["created_time", "username", "phone", "email", "sex"];
        const filterVal = ["created_time", "username", "phone", "email", "sex"];
